...
var i = class {
  ...
  matchesUrl(r2) {
    if (!this.definition.methods.includes("GET"))
      return false;
    const e2 = this.template.replace(/(\/?){([^}?]*)(\??)}/g, (t2, r3, e3, i3) => {
      var n3;
      const s3 = `(?<${e3}>${(null == (n3 = this.wheres[e3]) ? void 0 : n3.replace(/(^\^)|(\$$)/g, "")) || "[^/?]+"})`;
      return i3 ? `(${r3}${s3})?` : `${r3}${s3}`;
    }).replace(/^\w+:\/\//, ""), [i2, n2] = r2.replace(/^\w+:\/\//, "").split("?"), s2 = new RegExp(`^${e2}/?$`).exec(decodeURI(i2));
    if (s2) {
      for (const t2 in s2.groups)
        s2.groups[t2] = "string" == typeof s2.groups[t2] ? decodeURIComponent(s2.groups[t2]) : s2.groups[t2];
      return { params: s2.groups, query: (0, import_qs.parse)(n2) };
    }
    return false;
  }
  compile(t2) {
    return this.parameterSegments.length ? this.template.replace(/{([^}?]+)(\??)}/g, (r2, e2, i2) => {
      ...
      if (this.wheres[e2] && !new RegExp(`^${i2 ? `(${this.wheres[e2]})?` : this.wheres[e2]}$`).test(null != (s2 = t2[e2]) ? s2 : ""))
        throw new Error(`Ziggy error: '${e2}' parameter does not match required format '${this.wheres[e2]}' for route '${this.name}'.`);
...
var n = class extends String {
  constructor(t2, r2, n2 = true, s2) {
    if (super(), this.t = null != s2 ? s2 : "undefined" != typeof Ziggy ? Ziggy : null == globalThis ? void 0 : globalThis.Ziggy, this.t = e({}, this.t, { absolute: n2 }), t2) {
      if (!this.t.routes[t2])
        throw new Error(`Ziggy error: route '${t2}' is not in the route list.`);
      this.i = new i(t2, this.t.routes[t2], this.t), this.o = this.h(r2);
    }
  }
  toString() {
  ...
  current(t2, r2) {
    const { name: n2, params: s2, query: o2, route: h2 } = this.u();
    if (!t2)
      return n2;
    const u = new RegExp(`^${t2.replace(/\./g, "\\.").replace(/\*/g, ".*")}$`).test(n2);
    if ([null, void 0].includes(r2) || !u)
      return u;
    const a = new i(n2, h2, this.t);
    r2 = this.h(r2, a);
    const l = e({}, s2, o2);
    if (Object.values(r2).every((t3) => !t3) && !Object.values(l).some((t3) => void 0 !== t3))
      return true;
  ...
  l() {
    ...
    const { host: o2 = "", pathname: h2 = "", search: u = "" } = "undefined" != typeof window ? window.location : {};
    return { host: null != (t2 = null == (r2 = this.t.location) ? void 0 : r2.host) ? t2 : o2, pathname: null != (e2 = null == (i2 = this.t.location) ? void 0 : i2.pathname) ? e2 : h2, search: null != (n2 = null == (s2 = this.t.location) ? void 0 : s2.search) ? n2 : u };
  }
  get params() {
    const { params: t2, query: r2 } = this.u();
    return e({}, t2, r2);
  }
  has(t2) {
    return Object.keys(this.t.routes).includes(t2);
  }
  h(t2 = {}, r2 = this.i) {
    null != t2 || (t2 = {}), t2 = ["string", "number"].includes(typeof t2) ? [t2] : t2;
    const i2 = r2.parameterSegments.filter(({ name: t3 }) => !this.t.defaults[t3]);
    return Array.isArray(t2) ? t2 = t2.reduce((t3, r3, n2) => e({}, t3, i2[n2] ? { [i2[n2].name]: r3 } : "object" == typeof r3 ? r3 : { [r3]: "" }), {}) : 1 !== i2.length || t2[i2[0].name] || !t2.hasOwnProperty(Object.values(r2.bindings)[0]) && !t2.hasOwnProperty("id") || (t2 = { [i2[0].name]: t2 }), e({}, this.p(r2), this.$(t2, r2));
  }
  p(t2) {
    return t2.parameterSegments.filter(({ name: t3 }) => this.t.defaults[t3]).reduce((t3, { name: r2 }, i2) => e({}, t3, { [r2]: this.t.defaults[r2] }), {});
  }
  $(t2, { bindings: r2, parameterSegments: i2 }) {
    return Object.entries(t2).reduce((t3, [n2, s2]) => {
      if (!s2 || "object" != typeof s2 || Array.isArray(s2) || !i2.some(({ name: t4 }) => t4 === n2))
        return e({}, t3, { [n2]: s2 });
      if (!s2.hasOwnProperty(r2[n2])) {
        if (!s2.hasOwnProperty("id"))
          throw new Error(`Ziggy error: object passed as '${n2}' parameter is missing route model binding key '${r2[n2]}'.`);
        r2[n2] = "id";
      }
...
function h(t2) {
  if (!t2 && !globalThis.Ziggy && "undefined" == typeof Ziggy)
